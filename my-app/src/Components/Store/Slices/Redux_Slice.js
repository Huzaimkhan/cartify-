...
export const fetch_api = createAsyncThunk("fetchdata", async () => {
  const getdata = await axios.get("http://localhost:8000/naz_api_data", {
    crossOriginIsolated: true,
  });
  return getdata;
});

export const fetchsingle_api = createAsyncThunk(
  "fetchsingledata",
  async (id) => {
    const getsingledata = await axios.get(
      `http://localhost:8000/Singlecart_api/${id}`
    );
...
export const fetchsearch_api = createAsyncThunk(
  "fetchsearch",
  async (search) => {
    const getsearch = await axios.get(`http://localhost:8000/Search/${search}`);
    return getsearch;
  }
);
...
export const fetchprice_api = createAsyncThunk("fetchprice", async (price) => {
  const getprice = await axios.get(
    `http://localhost:8000/Searchprice/${price}`
  );
  return getprice;
});
...
export const { TotalProduct, add, sub, fitler, total, location } =
  Product_page_Slice.actions;
